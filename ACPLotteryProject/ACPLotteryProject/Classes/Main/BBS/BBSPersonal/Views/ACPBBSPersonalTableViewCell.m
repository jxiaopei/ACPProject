...
@interface ACPBBSPersonalTableViewCell()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation ACPBBSPersonalTableViewCell
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        [delectBtn setImage:[UIImage imageNamed:@"删除帖子"] forState:UIControlStateNormal];
...
//        [headerBtn addTarget:self action:@selector(didClickHeaderBtn:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [likeBtn setImage:[UIImage imageNamed:@"未关注"] forState:UIControlStateNormal];
        [likeBtn setImage:[UIImage imageNamed:@"关注"] forState:UIControlStateSelected];
        ...
        [collectBtn addTarget:self action:@selector(didClickCollcetBtn:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)didClickDelectBtn:(UIButton *)sender{
    if(self.didClickDelectBtnBlock){
        self.didClickDelectBtnBlock(sender);
    }
}

-(void)didClickLikeBtn:(UIButton *)sender{
    if(self.didClickLikeBtnBlock){
        self.didClickLikeBtnBlock(sender);
    }
}

-(void)didClickCommentBtn:(UIButton *)sender{
    if(self.didClickCommentBtnBlock){
        self.didClickCommentBtnBlock();
    }
}

-(void)didClickCollcetBtn:(UIButton *)sender{
    if(self.didClickCollectBtnBlock){
        self.didClickCollectBtnBlock(sender);
    }
}
...
-(void)setDataModel:(ACPBBSListDataModel *)dataModel{
    ...
    if(![dataModel.user_id isEqualToString:[ACPUserModel shareModel].uid]){
    ...
    _likeBtn.selected = dataModel.user_whether_love.integerValue;
    _collectBtn.selected = dataModel.user_whether_collection.integerValue;
}
